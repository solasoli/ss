	...
	class WPBakeryShortCode_DHVC_Form extends WPBakeryShortCode {
		...
		public function loadTemplate( $atts, $content = null ) {
			...
			if($form && $form->post_status === 'publish' && apply_filters('dhvc_form_display', true, $form->ID)){
				$output .= DHVCForm::form_custom_css($id);
				...
				if($wpb_custom_css = get_post_meta( $form->ID, '_wpb_post_custom_css', true )){
					$output .= '<style type="text/css">'.$wpb_custom_css.'</style>';
				}
				if($wpb_shortcodes_custom_css = get_post_meta( $form->ID, '_wpb_shortcodes_custom_css', true )){
					$output .= '<style type="text/css">'.$wpb_shortcodes_custom_css.'</style>';
				}
				...
				$form_msg = dhvc_form_the_messages($form->ID);
				$form_message = '';
				...
				$output .= '<form data-scroll_to_msg="'.apply_filters('dhvc_form_attr_scroll_to_msg', 1,$form->ID).'" data-ajax_reset_submit="'.apply_filters('dhvc_form_attr_ajax_reset_submit', 1,$form->ID).'" data-popup="'.(get_post_meta($form->ID,'_form_popup',true) ? '1':'0').'" autocomplete="off" data-use-ajax="'.(int)$use_ajax.'" method="' . $method . '" class="dhvcform dhvcform-' . $form->ID . '" enctype="'.apply_filters('dhvc_form_attr_enctype','multipart/form-data',$form->ID).'" target="'.apply_filters('dhvc_form_attr_target','_self',$form->ID).'" '.(!empty($action) ? ' action="' . $action . '"':'').'>' . "\n";
				if($method==='post'){
		...
		protected function loadTemplate( $atts, $content = null ) {
			if($this->shortcode !== 'dhvc_form_submit_button'){
				...
				if(empty($control_name))
					return __('Field name is required','dhvc-form');
			}
	...
	class WPBakeryShortCode_DHVC_Form_reCaptcha extends DHVC_Form_ShortCode {
		public function loadTemplate( $atts, $content = null ) {
			...
			if(!$recaptcha_public_key){
				echo __('ReCaptcha plugin needs a public key to be set in its parameters. Please contact a site administrator.','dhvc-form');
				return ;
			}
			return parent::loadTemplate($atts,$content);
		}
