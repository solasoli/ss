...
function dhvc_form_recipient_add(element){
	...
	var tmpl = jQuery(dhvcformAdminL10n.recipient_tmpl);
...
function dhvc_form_option_add(element){
	...
	var tmpl = jQuery(dhvcformAdminL10n.option_tmpl);
...
function dhvc_form_conditional_add(element){
	...
	conditional_table.append(dhvcformAdminL10n.conditional_tmpl);
	return false;
}
...
function dhvc_form_rate_option_add(element){
	...
	var tmpl = jQuery(dhvcformAdminL10n.rate_option_tmpl);
...
(function ($) {
	...
	$(document).on('click','.dhvc-form-entry-list .submitdelete',function(){
		return confirm(dhvcformAdminL10n.delete_confirm);
	});
	...
	$('#entry_note_form #add_note').click(function(e){
		...
		$('#entry_note_box').block({ message: null, overlayCSS: { background: '#fff url(' + dhvcformAdminL10n.plugin_url + '/assets/images/ajax-loader.gif) no-repeat center', opacity: 0.6 } });
