...
class WPVQShortcode {
	...
	*/
	 ...
	 * @return [type] [description]
	 */
	public static function choosePersonality($answers=array(), $multiplePersonalities=1) 
	{
		$local = false;
		if (!isset($_POST['answers']) || empty($_POST['answers'])) 
		{
			if (!$answers) {
				...
				$_POST['multiplePersonalities'] = $multiplePersonalities;
				$local = true;
			}
		}
		...
		arsort($_POST['answers']);
		$_POST['multiplePersonalities'] = intval($_POST['multiplePersonalities']);
		
		// Fetch player answer / question
		$i = 0;
		$results = array();
		foreach($_POST['answers'] as $id => $value)
		{
			$id 				=  intval($id);
			$appreciation 		=  new WPVQAppreciation();
			$appreciation		=  $appreciation->load($id);
			...
			$results[] = array(
				...
				'content' 	=> do_shortcode(htmlspecialchars_decode(stripslashes($appreciation->getContent()))),
			);

			$i++;
			if ($i >= $_POST['multiplePersonalities']) break;
		}
