...
abstract class WPVQGame {
	...
	*/
	 ...
	 * @return [type] [description]
	 */
	static public function getExtraOption($quizId, $optionName, $default = NULL)
	{
		global $wpdb;
	...
	public function _getExtraOption($optionName, $default = NULL) {
		return self::getExtraOption($this->id, $optionName, $default);
	}
	...
	public static function import($uploaded_path)
	{
		$upload_dir 		=  wp_upload_dir();
		$upload_url 		=  $upload_dir['url'];
		$upload_path 		=  $upload_dir['path'];
		$packname 			=  pathinfo($uploaded_path, PATHINFO_FILENAME); // filename without extension

		// Check file type
		if (!zipIsValid($uploaded_path)) {
			die('Wrong file.');
		}

		// Extract archive
		$zip 		=  new ZipArchive;
		$res 		=  $zip->open($uploaded_path);

		if ($res === TRUE) 
		{
			$temp_dir = 'wpvq_extract_' . time();

		    $dirname = dirname($uploaded_path) . '/' . $temp_dir;
		    mkdir($dirname, 0777, true);
			$zip->extractTo($dirname);

	    	$packdir = $dirname . '/' . dirname($zip->getNameIndex(0));

	    	$zip->close();
		} 
		else {
		    die('Error : can\'t upload  (7).');
		}

		// Get the json file
		$exportJson = wpvq_read_local_file($packdir . '/export.json');
		if ($exportJson === FALSE) { die('Error : can\'t upload (8).'); }
		// Parse raw to json
		...
		// Upload pictures
		foreach (glob("$packdir/*.{jpg,png,gif}", GLOB_BRACE) as $key => $file) 
		{
			$basename 	=  pathinfo($file, PATHINFO_BASENAME);
			$oldId 		=  pathinfo($file, PATHINFO_FILENAME);

			$newId 		=  custom_media_sideload_image("$packdir/$basename", false, true);
		...
		foreach ($exportJson['appreciations'] as $appreciation)
		{
			...
			{
				$newId 	=  custom_media_sideload_image("$packdir/$match", false, true);
