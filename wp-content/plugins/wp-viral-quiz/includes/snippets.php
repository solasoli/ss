 ...
 * If $local_img = true, you can sideload a local image already uploaded somewhere on the server
 * @param  string  $image_path [description]
 * @param  boolean $post_id   [description]
 * @return [type]             [description]
 */
function custom_media_sideload_image( $image_path = '', $post_id = false, $local_img = false) 
{
    ...
    preg_match( '/[^\?]+\.(jpe?g|jpe|gif|png)\b/i', $image_path, $matches );
...
/**
 * Smart get content function
 * @param  [type] $url [description]
 * @return [type]      [description]
 */
function wpvq_read_local_file($path) 
{
    $handle = fopen($path, "r");
