...
class EForm_AutoUpdate {
	...
	public function current_activation_status( $purchase_code ) {
		...
		if ( $this->get_activation_status( $purchase_code ) ) {
			$return = array(
				'activated' => true,
				'msg' => __( 'Your product eForm is activated and receiving automatic updates.', 'ipt_fsqm' ),
			);
			...
			$return = array(
				'activated' => false,
				'msg' => __( 'Your product eForm is not activated. Please enter your purchase code.', 'ipt_fsqm' ),
			);
		} else {
			$return = array(
				'activated' => false,
				'msg' => __( 'Your product eForm is activated for another domain. Activating it here will remove activation from the other domain.', 'ipt_fsqm' ),
			);
		}
		return $return;
	}
